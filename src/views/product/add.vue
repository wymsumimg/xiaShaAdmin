<template>
  <div class="add">
    <div class="add-title">发布商品</div>
    <div class="add-item" flex="cross:center">
      <div class="add-item-left">
        <span class="redColor">*</span> 商品类型
      </div>
      <div class="add-item-right" flex="cross:center main:center">
        <el-radio v-model="product.productType" :label="1" :disabled='id ? true : false'>智能家具</el-radio>
        <el-radio v-model="product.productType" :label="2" :disabled='id ? true : false'>宽带业务</el-radio>
      </div>
    </div>
    <div class="add-item" flex="cross:center">
      <div class="add-item-left">
      ...
      <div class="add-item-right" flex="cross:center main:center">
        <el-radio v-model="product.productType" :label="1" :disabled='id ? true : false'>联通宽带</el-radio>
        <el-radio v-model="product.productType" :label="2" :disabled='id ? true : false'>移动宽带</el-radio>
        <el-radio v-model="product.productType" :label="3" :disabled='id ? true : false'>电信宽带</el-radio>
        <el-radio v-model="product.productType" :label="4" :disabled='id ? true : false'>华数宽带</el-radio>
        <el-radio v-model="product.productType" :label="5" :disabled='id ? true : false'>长城宽带</el-radio>
      </div>
    </div>
    <div class="add-item" flex="cross:center">
      <div class="add-item-left">有无光猫</div>
      <div class="add-item-right" flex="cross:center main:center">
        <el-switch v-model="product.hotFlag" :active-value="1" :inactive-value="0" active-color="#FF6E39" inactive-color="#E5E5E5">
        </el-switch>
      </div>
    </div>
    <div class="add-item" flex="cross:center">
      <div class="add-item-left">光猫押金</div>
      <div class="add-item-right" flex="cross:center main:center">
        <el-input @change="handleInput3" v-model="product.productPrice" class="add-item-inputValue" style="width: 100px; margin-right: 16px;"></el-input>
        <span style="font-size: 14px; color: #C0C3CF; padding-right: 12px;">元</span>
        <span style="font-size: 12px; color: #C0C3CF;">请输入0，1，2…等大于等于0的正整数。</span>
      </div>
    </div>

    <div class="add-item" flex="cross:center">
      <div class="add-item-left">
        <span class="redColor">*</span> 商品展示图
      </div>
    ...
    <div class="add-item" flex="cross:center">
      <div class="add-item-left">
        <span class="redColor">*</span> 商品主图/视频
      </div>
      <div class="add-item-right" flex="cross:center">
        <div style="position: relative;" v-for="(item, index) in resources2" :key="index">
          <l-upload :imgUrl.sync='item.resource' :spec='bannerSpec' name='上传主图'></l-upload>
          <img src="./images/del.png" btn class="add-item-rightImg" @click='resources2.splice(key, 1)' v-if="item.resource != ''" />
        </div>
        <div v-for="(item, index) in resources1" style="position: relative;" :key="index">
          <l-upload name='上传视频' :imgUrl.sync='item.resource' type='2'></l-upload>
          <img src="./images/del.png" btn class="add-item-rightImg" @click='$set(resources1[0], "resource", "")' v-if="item.resource != ''"
          />
        </div>
      </div>
    </div>
    ...
    <div class="add-item">
      <div class="add-item-left">
        <span class="redColor">*</span> 商品名称
      </div>
      <div class="add-item-right">
        <div class="add-item-input" flex="cross:center">
          <el-input v-model="product.productName" placeholder='输入内容' class="add-item-inputValue" :maxlength='25'></el-input>
        </div>
      </div>
    </div>
    <div class="add-item">
      <div class="add-item-left">
        <span class="redColor">*</span> 商品价格
      </div>
      <div class="add-item-right">
        <div class="add-item-input" flex="cross:center">
          <el-input @change="handleInput3" v-model="product.productPrice" placeholder='¥ 价格' class="add-item-inputValue"></el-input>
        </div>
      </div>
    </div>
    ...
    <div class="add-item">
      <div class="add-item-left">
        <span class="redColor">*</span> 添加规格
      </div>
      <div class="add-item-right">
        <div v-for='(item, index) in skus' :key="index">
          <div flex="main:justify" style="width: 100%;margin-bottom: 25px;">
            <div class="add-item-addPrice" flex="cross:center">
              <span>规格名称： {{item.skuName}}</span>
              <p>该规格原价： ¥ {{item.skuPrice}}</p>
              <i class="el-icon-close" btn @click='skus.splice(key, 1)'></i>
            </div>
            <div class="add-item-add" flex="cross:center main:center" @click="showDialog(1,item.skuPrice,key)" btn style="margin-left: 30px;">
              <i class="el-icon-plus"></i>添加地区价格
            </div>
          </div>
          <div class="add-item" v-show="item.regionPrices && item.regionPrices.length > 0" style="margin-bottom: 50px;">
            <el-table :data="item.regionPrices" style="width: 100%;border-top: 1px solid #F0F0F0;">
              <el-table-column label="规格" align="center" prop="item">
                {{item.skuName}}
              </el-table-column>
              <el-table-column label="所在小区" align="center" prop="item">
                <slot slot-scope="scope">
                  <p>{{scope.row.regionName}}</p>
                </slot>
              </el-table-column>
              <el-table-column label="优惠后价格" align="center" width="150" style="color: #FF6E39;">
                <slot slot-scope="scope">
                  <p>￥ {{scope.row.price}}</p>
                </slot>
              </el-table-column>
              <el-table-column label="操作" align="center" width="150" prop="item">
                <slot slot-scope="scope">
                  <p @click="deleteTableData(item, scope.$index)" style="color: #3A84FF;">删除</p>
                </slot>
              </el-table-column>
            </el-table>
          </div>
        ...
        <div class="add-item-add" flex="cross:center main:center" btn @click="showDialog(2)" :style="{'margin': skus.length > 0 ? '0 auto' : '0'}">
          <i class="el-icon-plus"></i>{{skus.length > 0 ? '继续添加规格' : '添加规格'}}
        </div>
      </div>
    </div>

    <div class="add-item">
      <div class="add-item-left">是否推荐</div>
      <div class="add-item-right" flex="cross:center main:center">
        <div class="add-item-input">
          <el-switch v-model="product.recommFlag" :active-value="1" :inactive-value="0" active-color="#FF6E39" inactive-color="#E5E5E5">
          </el-switch>
        </div>
      </div>
    </div>
    <div class="add-item">
      <div class="add-item-left">是否热销</div>
      <div class="add-item-right" flex="cross:center main:center">
        <div class="add-item-input">
          <el-switch v-model="product.hotFlag" :active-value="1" :inactive-value="0" active-color="#FF6E39" inactive-color="#E5E5E5">
          </el-switch>
    ...
    <div class="add-item" flex="cross:center">
      <div class="add-item-left">分佣总额</div>
      <div class="add-item-right" flex="cross:center main:center">
        <el-input @change="handleInput3" v-model="product.productPrice" class="add-item-inputValue" style="width: 100px;margin-right: 16px;"></el-input>
        <span style="font-size: 14px; color: #C0C3CF;">元</span>
      </div>
    </div>
    <div class="add-item">
      <div class="add-item-left">商品详情</div>
      <div class="add-item-right">
        <quill-editor v-model="content" ref="myQuillEditor" :options="editorOption" @change="onEditorChange($event)" style='width: 660px;'>
        </quill-editor>
      </div>
    </div>
    <div class="add-item">
      ...
      <div class="add-item-right" flex="cross:center">
        ...
        <div class="add-btn2" btn flex="cross:center main:center" @click="upperOrLower">发布上架</div>
      </div>
    </div>
    <l-dialog :dialogVisible.sync='dialogVisible' :title='dialog.title' :width='dialog.width' :center="dialog.type!='3'">
      <div v-if="dialog.type=='1'" flex="cross:center dir:top">
        ...
        <div class="dialog-item" flex="main:center cross:center">
          <span>小区折扣后价格</span>
          <el-input @change="handleInput" v-model="priceProportion" placeholder='请输入新的折扣价格' class="add-item-inputValue" style='width: 355px;'></el-input>
        </div>
        <div class="dialog-item" flex="main:center cross:center">
          <span>选中小区</span>
          <el-select v-model="PriceData" placeholder="请选择" style='width: 355px;' value-key="id">
            <el-option v-for="(item, key) in PricesList" :key="item.id" :label="item.regionName" :value="item">
            </el-option>
          </el-select>
        </div>
        <div class="add-btn" btn flex="cross:center main:center" style="width: 360px;margin-top: 30px;border-radius: 10px;" @click="saveDialog(1)">保存</div>
      </div>
      <div v-if="dialog.type=='2'" flex="cross:center dir:top">
        <div class="dialog-item" flex="main:center cross:center">
          <span>规格名称</span>
          <el-input v-model="skuData.skuName" placeholder='输入规格名称' class="add-item-inputValue" style='width: 355px;'></el-input>
        </div>
        <div class="dialog-item" flex="main:center cross:center">
          <span>该规格原价</span>
          <el-input @change="handleInput2" v-model="skuData.skuPrice" placeholder='¥ 价格' class="add-item-inputValue" style='width: 355px;'></el-input>
        </div>
        <div flex="main:center cross:center">
          <div class="add-btn" btn flex="cross:center main:center" style="width: 200px;margin-top: 0;border-radius: 10px;" @click="saveDialog(2, false)">确定</div>
          <div class="add-btn2" btn flex="cross:center main:center" style="width: 200px;margin-top: 0;border-radius: 10px;" @click="saveDialog(2, true)">继续添加</div>
        </div>
      </div>
      <div v-if="dialog.type=='3'" flex="cross:center dir:top">
        <el-table ref='multipleTable' :data="assocList" @selection-change="handleSelectionChange" style="width: 100%;border-top: 1px solid #F0F0F0;">
          ...
          <el-table-column prop="productSales" align="center" label="销售额" width="200">
            <slot slot-scope="scope">
              <span>{{scope.row.productSales | getMoney(2)}}</span>
            </slot>
          </el-table-column>
        </el-table>
